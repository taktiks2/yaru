use crate::{
    json::{load_json, save_json},
    task::Task,
};
use anyhow::Result;
use std::path::PathBuf;

/// タスクリポジトリのトレイト
/// データの永続化方法を抽象化し、異なる実装（JSON、SQLiteなど）を切り替え可能にする
pub trait TaskRepository {
    /// タスクリストを読み込む
    fn load_tasks(&self) -> Result<Vec<Task>>;

    /// タスクリストを保存する
    fn save_tasks(&self, tasks: &[Task]) -> Result<()>;

    /// 次のIDを取得する
    fn find_next_id(&self, tasks: &[Task]) -> u64 {
        tasks.iter().map(|task| task.id).max().unwrap_or(0) + 1
    }

    /// データファイルが存在することを確認（必要に応じて初期化）
    fn ensure_data_exists(&self) -> Result<()>;
}

/// JSON形式でタスクを保存するリポジトリ実装
pub struct JsonTaskRepository {
    file_path: PathBuf,
}

impl JsonTaskRepository {
    /// 新しいJsonTaskRepositoryインスタンスを作成
    pub fn new(file_path: impl Into<PathBuf>) -> Self {
        Self {
            file_path: file_path.into(),
        }
    }
}

impl TaskRepository for JsonTaskRepository {
    fn load_tasks(&self) -> Result<Vec<Task>> {
        load_json(&self.file_path)
    }

    fn save_tasks(&self, tasks: &[Task]) -> Result<()> {
        save_json(&self.file_path, tasks)
    }

    fn ensure_data_exists(&self) -> Result<()> {
        if !self.file_path.exists() {
            save_json(&self.file_path, &Vec::<Task>::new())?;
        }
        Ok(())
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::task::{Priority, Status};
    use std::fs;
    use tempfile::TempDir;

    fn setup_test_dir() -> TempDir {
        tempfile::tempdir().unwrap()
    }

    #[test]
    fn test_ensure_data_exists_creates_file() {
        let test_dir = setup_test_dir();
        let test_file = test_dir.path().join("tasks.json");

        // ファイルが存在しないことを確認
        assert!(!test_file.exists());

        // リポジトリを作成してファイルを初期化
        let repo = JsonTaskRepository::new(&test_file);
        repo.ensure_data_exists().unwrap();

        // ファイルが作成されたことを確認
        assert!(test_file.exists());

        // 空のJSONリストが保存されていることを確認
        let content = fs::read_to_string(&test_file).unwrap();
        assert_eq!(content, "[]");
    }

    #[test]
    fn test_load_tasks_empty_list() {
        let test_dir = setup_test_dir();
        let test_file = test_dir.path().join("tasks.json");

        // 空のJSONファイルを作成
        fs::write(&test_file, "[]").unwrap();

        // リポジトリを使って読み込み
        let repo = JsonTaskRepository::new(&test_file);
        let tasks = repo.load_tasks().unwrap();

        // 空のリストが返されることを確認
        assert_eq!(tasks.len(), 0);
    }

    #[test]
    fn test_save_and_load_tasks() {
        let test_dir = setup_test_dir();
        let test_file = test_dir.path().join("tasks.json");

        let tasks = vec![
            Task::new(1, "テストタスク1", "", Status::Pending, Priority::Medium),
            Task::new(2, "テストタスク2", "", Status::Completed, Priority::Medium),
        ];

        // リポジトリを使って保存
        let repo = JsonTaskRepository::new(&test_file);
        repo.save_tasks(&tasks).unwrap();

        // ファイルが作成されたことを確認
        assert!(test_file.exists());

        // 保存したTaskを読み込み
        let loaded_tasks = repo.load_tasks().unwrap();

        // 正しく保存・読み込みできたことを確認
        assert_eq!(loaded_tasks.len(), 2);
        assert_eq!(loaded_tasks[0].id, 1);
        assert_eq!(loaded_tasks[0].title, "テストタスク1");
        assert_eq!(loaded_tasks[1].id, 2);
        assert_eq!(loaded_tasks[1].title, "テストタスク2");
    }

    #[test]
    fn test_find_next_id_empty_list() {
        let test_dir = setup_test_dir();
        let test_file = test_dir.path().join("tasks.json");
        let repo = JsonTaskRepository::new(&test_file);

        let tasks: Vec<Task> = vec![];
        let next_id = repo.find_next_id(&tasks);
        assert_eq!(next_id, 1);
    }

    #[test]
    fn test_find_next_id_with_existing_tasks() {
        let test_dir = setup_test_dir();
        let test_file = test_dir.path().join("tasks.json");
        let repo = JsonTaskRepository::new(&test_file);

        let tasks = vec![
            Task::new(1, "タスク1", "", Status::Pending, Priority::Medium),
            Task::new(3, "タスク3", "", Status::Pending, Priority::Medium),
            Task::new(2, "タスク2", "", Status::Pending, Priority::Medium),
        ];
        let next_id = repo.find_next_id(&tasks);
        assert_eq!(next_id, 4);
    }

    #[test]
    fn test_find_next_id_with_single_task() {
        let test_dir = setup_test_dir();
        let test_file = test_dir.path().join("tasks.json");
        let repo = JsonTaskRepository::new(&test_file);

        let tasks = vec![Task::new(
            5,
            "タスク",
            "",
            Status::Pending,
            Priority::Medium,
        )];
        let next_id = repo.find_next_id(&tasks);
        assert_eq!(next_id, 6);
    }

    #[test]
    fn test_repository_trait_with_custom_path() {
        let test_dir = setup_test_dir();
        let test_file = test_dir.path().join("custom_tasks.json");

        // カスタムパスでリポジトリを作成
        let repo = JsonTaskRepository::new(&test_file);

        // データが存在することを確認
        repo.ensure_data_exists().unwrap();
        assert!(test_file.exists());

        // Taskを保存
        let tasks = vec![Task::new(
            1,
            "カスタムパステスト",
            "",
            Status::Pending,
            Priority::Medium,
        )];
        repo.save_tasks(&tasks).unwrap();

        // Taskを読み込み
        let loaded_tasks = repo.load_tasks().unwrap();
        assert_eq!(loaded_tasks.len(), 1);
        assert_eq!(loaded_tasks[0].title, "カスタムパステスト");
    }
}
